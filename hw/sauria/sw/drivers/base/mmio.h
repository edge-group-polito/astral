...
static inline uint64_t mmio_region_read64(mmio_region_t base, ptrdiff_t offset) {
  return ((volatile uint64_t *)base.base)[offset / sizeof(uint64_t)];
}

static inline void mmio_region_write64(mmio_region_t base, ptrdiff_t offset, uint64_t value) {
  ((volatile uint64_t *)base.base)[offset / sizeof(uint64_t)] = value;
}

static inline void mmio_region_write32_rmw64(mmio_region_t base, ptrdiff_t offset, uint32_t value) {
  ptrdiff_t aligned = offset & ~(ptrdiff_t)0x7; // 8-byte aligned address
  uint64_t cur = mmio_region_read64(base, aligned);

  if (offset & 0x4) {
    // Upper 32 bits of the 64-bit word
    cur = (cur & 0x00000000FFFFFFFFull) | ((uint64_t)value << 32);
  } else {
    // Lower 32 bits of the 64-bit word
    cur = (cur & 0xFFFFFFFF00000000ull) | (uint64_t)value;
  }

  mmio_region_write64(base, aligned, cur);
}
