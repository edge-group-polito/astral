...
void sauria_reg0x204(const sauria_t *sauria, uint16_t wei_reps, uint8_t neg_thres, uint16_t res_mask, uint8_t appr_mask) {
  uint32_t reg = 0;
  uint16_t wei_reps_shifted = wei_reps >> 2;
...
void sauria_reg0x208(const sauria_t *sauria, uint8_t appr_mask, bool sram_deepsleep, bool sram_powergate) {
  uint32_t reg = 0;
  uint8_t appr_mask_shifted = appr_mask >> 6;
...
void sauria_reg0x804(const sauria_t *sauria, uint16_t cxstep, uint16_t cklim, uint16_t ckstep) {
  uint32_t reg = 0;
  uint16_t cxstep_shifted = cxstep >> 4;
...
void sauria_reg0x808(const sauria_t *sauria, uint16_t ckstep, uint16_t til_cylim, uint16_t til_cystep) {
  uint32_t reg = 0;
  uint16_t ckstep_shifted = ckstep >> 8;
...
void sauria_reg0x80C(const sauria_t *sauria, uint16_t til_cystep, uint16_t til_cklim, uint16_t til_ckstep, uint8_t inactive_cols) {
  uint32_t reg = 0;
  uint8_t til_cystep_shifted = til_cystep >> 12;
  ...
  reg = bitfield_field32_write(reg, SAURIA_CFG_OUT_TIL_CKSTEP_FIELD, til_ckstep);
  reg = bitfield_field32_write(reg, SAURIA_CFG_OUT_INACTIVE_COLS_1_FIELD, inactive_cols);
...
void sauria_reg0x810(const sauria_t *sauria, uint8_t inactive_cols, bool preload_en) {
  uint32_t reg = 0;
  uint8_t inactive_cols_shifted = inactive_cols >> 2;
  reg = bitfield_field32_write(reg, SAURIA_CFG_OUT_INACTIVE_COLS_2_FIELD, inactive_cols_shifted);
